...
public interface UserAccountRepository extends ReactiveCrudRepository<UserAccount, Integer> {
    @Query("SELECT us.max_aqi, us.longitude, us.latitude, ua.id, ua.email, ua.first_name, ua.last_name, ua.apple_id, ua.is_safe " +
            "FROM user_account AS ua " +
            "INNER JOIN user_settings AS us on ua.id = us.user_id " +
            "WHERE ((is_safe IS TRUE OR is_safe IS NULL) AND (last_checked < :earlierDate OR last_checked IS NULL)) " +
            "AND us.max_aqi IS NOT NULL " +
            "AND (us.longitude IS NOT NULL AND us.latitude IS NOT NULL) " +
    ...
    @Query("SELECT us.max_aqi, us.longitude, us.latitude, ua.id, ua.email, ua.first_name, ua.last_name, ua.apple_id, ua.is_safe " +
            "FROM user_account AS ua " +
            "INNER JOIN user_settings AS us on ua.id = us.user_id " +
            "WHERE ((is_safe IS FALSE) " +
            "AND (last_checked < :earlierDate OR last_checked IS NULL)) " +
            "AND us.max_aqi IS NOT NULL " +
            "AND (us.longitude IS NOT NULL AND us.latitude IS NOT NULL) " +
            "ORDER BY last_checked ASC " +
            "LIMIT 3 FOR UPDATE")
